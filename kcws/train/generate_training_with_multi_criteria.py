...
def processToken(token, sentence, tag_vob, count):
	label_tuple = ('</as>', '</cityu', '</cnb>', '</ctn>', '</msr>', '</pku>', '</sxu>', '</udc>', '</wtb>', '</zx>')
	if token.startswith(label_tuple) and token.endswith('/S'):
		'''
		</pku>/S case
		'''
		tok = token.split('/')
		assert(len(tok) == 3), '{} {}'.format(count, token)
		word = '/'.join([tok[0], tok[1]])
		tag = tok[2]
	else:
		tok = token.split('/')
		...
		if len(tok) == 2:
			...
			print('ERROR {} {}'.format(count, token))
...
def processLine(line, out, vec_vob, tag_vob, count):
	line = line.strip()
	line = line.split('\t')

	sentence = Sentence()
	for token in line:
		processToken(token, sentence, tag_vob, count)
...
def split_train_testing(data_path):
	def get_traingin_testing_data(data_path):
		with open(data_path, 'r') as fp:
			...
			shuffle(all_text)
			# training_text_ = all_text[: (95 * file_len) / 100]
			# print(file_len, len(training_text_), (95 * file_len) / 100)
			training_text = all_text[: (95 * file_len) // 100]
			testing_text = all_text[(95 * file_len) // 100:]
...
def main(argc, argv):
	...
	with open(corpusPath, 'r') as fp:
		...
		for count, line in enumerate(all_text):
			...
			processLine(line, out, vec_vob, tagVob, count)
